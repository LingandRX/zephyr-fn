...
本模块只负责备份文件以及订阅/分类数据的序列化。所有面向用户的导出都必须
显式传入 ``user_id``；全量导出只允许内部调用时显式传入 ``include_all=True``。

数据库层目前没有公开事务 API，因此导入优先使用 db 模块已经建立的连接和锁，
在一个 ``BEGIN IMMEDIATE`` 事务中写入；如果运行环境没有暴露该连接，则降级到
现有 raw API，并在结果中标记 ``atomic=False``。
"""
...
# 导入是 HTTP 接口可直接触发的操作，限制规模可以避免一次性把进程内存耗尽。
...
# 这些列与当前 db.py 的 subscriptions 表一致。不要把导入 JSON 的任意键直接
# 拼接进 SQL；导入始终只写入这组固定列。
...
def _resolve_export_scope(user_id: str | None, include_all: bool) -> str | None:
    ...
    ``None`` 不再隐含“全量用户”。这样旧调用如果没有完成主任务的
    user_id 传递，只会得到明确错误，而不会意外泄露全库数据。
    """
    if include_all:
...
def export_json(user_id: str | None = None, *, include_all: bool = False) -> dict:
    ...
    Args:
        user_id: 用户范围。普通导出必须传入。
        include_all: 仅供内部系统备份使用的全量导出开关。
    """
    scope = _resolve_export_scope(user_id, include_all)
...
def export_csv(user_id: str | None = None, *, include_all: bool = False) -> str:
    ...
    ``ID``、``开始日期``和自定义周期列都是向后兼容的追加列：旧 CSV 没有
    这些列也仍然可以导入；新 CSV 可以在往返导入时保留外部 ID、周期锚点和
    自定义周期参数。
    """
...
def _parse_date(value: Any, field: str, *, default: str | None = None,
    ...
    text = str(value).strip()
    candidate = text
    # 数据库时间戳可能是 RFC3339；日期字段只接受其日期部分，且必须明确
    # 以 T/空格分隔，避免把任意脏字符串截成“看起来像日期”。
...
def _clean_timestamp(value: Any, default: str) -> str:
    ...
    # 只做长度/非空校验，保留旧备份中的 RFC3339 字符串；排序时仍使用原文。
...
def _normalize_imported_sub(raw: dict, user_id: str,
    ...
    """将 JSON/CSV 行规范化为 subscriptions 表的一整行。"""
    ...
    # 缺少 next_due_date 时按现有领域逻辑补一周期，避免导入后订阅静默失效。
...
def _transaction() -> Iterator[Any]:
    """优先使用 db 的真实连接做原子事务；没有连接时 yield None。"""
...
def _insert_fallback(categories: list[dict], subscriptions: list[dict]) -> None:
    """兼容没有暴露 db._conn 的测试/替代实现。"""
    for category in categories:
        db.insert_category_raw(category)
    for sub in subscriptions:
        # ID 已经在规划阶段解决冲突；此处只能依赖旧 raw API 的提交行为。
...
def _load_existing_subscriptions(user_id: str) -> tuple[dict[str, dict], set[str]]:
    ...
    # 使用 db API 的去重键作为兼容补充，防止某个替代 db 实现的 raw 行缺字段。
    try:
        keys.update(str(key).lower() for key in db.get_subscription_dedup_keys(target_user))
    except Exception:  # noqa: BLE001 - 兼容旧/轻量 db stub
        pass
    return by_id, keys
...
def _load_category_state(user_id: str) -> tuple[dict[str, dict], dict[str, dict], dict[str, dict]]:
    ...
    target_categories = [c for c in all_categories if c.get("user_id") == target_user]
...
def _plan_categories(user_id: str, categories_in: list[Any],
    ...
    返回 ``source_id -> target_id``、``name -> target_id``、待插入分类以及冲突数。
    分类 ID 是全局主键；遇到别的用户占用的外部 ID 时，绝不覆盖，改用目标用户
    已有同名分类或生成新 ID。
    """
    target_user = _require_user_id(user_id)
    ...
    def ensure_category(name: str, source_id: str | None = None,
        ...
        if target_id and target_id in global_by_id:
            # 只有属于目标用户且名称一致的同 ID 才能复用；其他用户占用，
            # 或目标用户已有同 ID 但名称不同，都不能被外部分类覆盖。
...
def _resolve_category_id(source_id: Any, source_to_target: dict[str, str],
    ...
    if target:
        return target
    # 仅当外部 ID 已经属于当前用户时才复用；不能把另一个用户的分类 ID
    # 直接写入当前用户订阅。
    try:
...
def _prepare_subscription_items(raw_items: list[Any], user_id: str,
    ...
    for index, raw in enumerate(raw_items, start=1):
        ...
        try:
            ...
            # category_id 只作为外部引用，后续一定经过映射；绝不信任 source user_id。
            ...
            if source_id is None:
                ...
                # 任何已存在 ID 都不能 INSERT OR REPLACE；即使是当前用户，
                # 导入也不应无提示地覆盖现有记录。
                sub["id"] = db.new_id()
                id_conflicts += 1
            else:
...
def import_from_json(json_data: str, user_id: str) -> dict:
    ...
    try:
        ...
        # 订阅可能引用 categories 数组中没有的 category_id；上述解析已将其置空。
    ...
    for row_index, row in enumerate(rows, start=2):
        try:
            ...
            raw_items.append({
                ...
                # 旧 CSV 没有开始日期时，退回使用首次付款日；再缺失则由
                # 规范化逻辑使用今天，保持历史文件的兼容行为。
                "start_date": (
                ...
                # CSV 没有 auto_renew 列；续费策略由规范化逻辑决定。
...
def merge_from_database(source_path: str, user_id: str) -> str:
    """按 ``id/updated_at`` 合并 SQLite 备份，且所有记录归属目标 user_id。

    source 中属于其他用户的 ID 不会覆盖本库记录，而是生成新的目标 ID。
    同一用户的同 ID 记录只有在源 updated_at 更新时才会显式 UPDATE。
    """
    try:
        target_user = _require_user_id(user_id)
    except ValueError as exc:
        ...
        for sub in normalized:
            ...
            if current is not None and current.get("user_id") == target_user:
                if _is_newer(sub, current):
                    ...
                    skipped += 1
                continue
            # 其他用户占用相同外部 ID，或者同一备份内重复 ID：重映射后插入。
            sub["id"] = db.new_id()
