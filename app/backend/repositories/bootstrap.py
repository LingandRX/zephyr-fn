        ...
        for stmt in statements:
            ...
            if lower.startswith("alter table"):
                ...
                column = (
                    tokens[5]
                    if len(tokens) > 5
                    and tokens[3].lower() == "add"
                    and tokens[4].lower() == "column"
                    else ""
                )
...
def _ensure_notification_schema(conn: Any) -> None:
    ...
    for index in existing:
        ...
        columns = [
            row._mapping["name"]
            for row in conn.execute(text(f'PRAGMA index_info("{index_name}")')).all()
        ]
...
def _dedupe_categories(conn: Any) -> None:
    ...
    rows = conn.execute(text("SELECT id, user_id, name FROM categories ORDER BY rowid ASC")).all()
    ...
    for cat_id, kept_id in doomed:
        conn.execute(
            text(
                "UPDATE subscriptions SET category_id=:kept, updated_at=:now WHERE category_id=:cat"
            ),
...
def _seed_default_categories(conn: Any) -> None:
    ...
    conn.execute(
        text("INSERT OR IGNORE INTO seeded_users (user_id, seeded_at) VALUES (:u, :t)"),
        {"u": "local", "t": now_utc()},
    )
