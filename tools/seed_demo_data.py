...
def build_demo_data() -> dict:
    ...
    subs = [
        ...
        sub("爱奇艺黄金会员", 25, "CNY", "month", "流媒体", "2026-08-28", start="2025-09-01"),
        sub(
            "YouTube Premium",
            22.99,
            "USD",
        ...
        sub("网易云音乐黑胶VIP", 15, "CNY", "month", "音乐", "2026-08-25", start="2025-04-01"),
        sub(
            "Apple Music 学生版",
            5.99,
            "USD",
        ...
        sub("印象笔记专业版", 98, "CNY", "year", "办公", "2026-10-01", start="2025-10-01"),
        sub(
            "PlayStation Plus 三档", 308, "CNY", "quarter", "游戏", "2026-11-01", start="2025-11-01"
        ),
        sub(
            "Xbox Game Pass Ultimate", 79, "CNY", "month", "游戏", "2026-08-19", start="2025-10-20"
        ),
        sub("Keep 会员", 25, "CNY", "month", "健身", "2026-08-22", start="2026-01-10"),
        sub("京东 PLUS 会员", 149, "CNY", "year", "电商会员", "2027-01-15", start="2026-01-15"),
        sub("淘宝 88VIP", 888, "CNY", "year", "电商会员", "2026-12-31", start="2025-12-31"),
        sub(
            "Adobe Creative Cloud 摄影",
            68,
            "HKD",
            "month",
            "办公",
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        default=str(REPO_ROOT / "data" / "demo-data.json"),
    ...
    print(
        f"已生成 JSON 导出文件: {out}（订阅 {len(data['subscriptions'])} 条，"
    ...
    if args.db:
        ...
        result = import_from_json(json.dumps(data, ensure_ascii=False), args.user)
        print(
            f"已灌入数据库 {args.db}: 新增订阅 {result['success_count']} 条，"
